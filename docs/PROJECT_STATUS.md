...
- Booking funnel používa Jakubov Google Calendar `Konzultácie` ako zdroj dostupnosti na stagingu aj produkcii. Pri `POST /api/book` treba ešte urobiť kontrolovaný live smoke, pretože vytvorí reálny kalendárový event.
...
- Overiť kontrolovaným smoke testom, že `/api/book` vytvorí event v Google kalendári `Konzultácie`.
