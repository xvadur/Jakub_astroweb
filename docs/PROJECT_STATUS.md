...
Aktuálny booking stav k 17. júnu 2026: komunikácia wizardu s Google Calendar a tvorba eventov bola overená na Adamovom/test účte. Produkčné secrets sú podľa aktuálneho stavu nahodené okrem Jakubovho finálneho kalendára a mailu. Jakubov Google Calendar/mail ostáva externý onboarding krok na osobnom stretnutí.
...
BOSEN služby sú pre Jakubov web formulované opatrne: Jakub koordinuje ocenenie, prípravu nehnuteľnosti, marketing, prácu s kupujúcimi, právny/finančný/katastrálny servis a podľa situácie prenájom alebo správu cez BOSEN/partnerov. Jakub sa nekomunikuje ako realitný investor, výkupný produkt ani samostatný poskytovateľ právnych alebo finančných služieb.
...
- Sekcia služieb bola upravená podľa verejných BOSEN podkladov a briefu `docs/BOSEN_SERVICES_BRIEF_FOR_JAKUB_2026-06-16.md`: jadrom sú nacenenie/stratégia, príprava, marketing, kupujúci/obhliadky/vyjednávanie, právny/finančný/katastrálny servis cez zázemie a prenájom/správa ako alternatíva.
...
- Supabase media workflow bol pripravený 16. júna 2026:
  - private Storage bucket `jakub-media`,
  - rozšírené `public.media` metadata pre Telegram/storage,
  - `tasks.property_id` pre cross-referenced follow-upy,
  - `notes.entity_type` podporuje aj `media`,
  - OpenClaw tool `ops/openclaw/tools/supabase-media.mjs`:
    - `media.saveTelegramPhoto`,
    - `media.saveLocalFile`,
    - `media.createPropertyDraft`,
    - `media.ingestPropertyMedia`.
  - Agentové pravidlo: fotky z Telegramu sa najprv ukladajú do Supabase Storage a `public.media`; verejný web dostane iba schválené optimalizované assets cez `public/images/listings/<slug>/`.
...
- Runtime agent workspace má core docs a pracovné priečinky pre property workflow:
  ...
  - `media-inbox`,
  - `web-patches`.
- Aktuálny OpenClaw blocker pre maklérskeho agenta: ešte treba spustiť plný staging booking E2E po zapnutí hooku, pridať Access/service-token ochranu hook hostname a napojiť failed runs/admin cases do monitoringu. Potom treba otestovať reálny Telegram inbound od Jakuba. HighLevel `401` z older runbookov je historická stopa, nie aktuálny CRM smer.
- Backend audit a aktuálny backlog k 17. júnu 2026 je v `docs/BACKEND_SYSTEM_AUDIT_2026-06-17.md`.
...
- Booking má bezpečný minimálny predstih 120 minút (`BOOKING_MIN_LEAD_MINUTES=120`), aby návštevník nevytváral konzultácie prakticky okamžite pred termínom.
...
- Historická kontrola zo 6. júna 2026 hovorila, že produkčný Worker nemal secrets. Aktuálny manuálny stav od Adama 17. júna 2026: produkčné secrets sú nahodené okrem Jakubovho kalendára a mailu. Tento rozdiel treba pri najbližšej technickej kontrole overiť cez Wrangler/Cloudflare dashboard bez vypisovania secret hodnôt.
