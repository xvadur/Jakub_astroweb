...
Web je Astro stránka nasadená cez Cloudflare Workers Static Assets. Rezervácia konzultácie je samostatný wizard na `/rezervacia/` s Cloudflare Worker API backendom. Produkčný Worker už zapisuje prijaté rezervácie do Supabase CRM a posiela Telegram notifikáciu cez Jakubov bot. Kým nie sú nastavené Google Calendar secrets, booking API beží v mock calendar režime; po doplnení Google OAuth bude čítať dostupnosť cez Google Calendar `freeBusy` a zapisovať rezervácie cez `events.insert`. Potvrdzovací email cez Resend je v kóde pripravený, ale produkčne vypnutý, kým nebude overená doména a platný Resend API key.
...
- Worker je pripravený na Google Calendar sync a Resend potvrdzovacie emaily cez Cloudflare env/secrets.
...
- Pre Google Calendar booking ešte nastaviť Jakubove Google Calendar secrets:
...
- Overiť, že produkčný `/api/book` po Google Calendar setup-e vráti `bookingStatus: "calendar_created"`.
