...
function leadLines(payload) {
  ...
  return [
    ...
    `Lokalita overená Google: ${payload.lokalita_overena === "true" ? "áno" : "nie"}`,
    `Google Place ID: ${clean(payload.lokalita_place_id) || "-"}`,
    `GPS: ${clean(payload.lokalita_lat) && clean(payload.lokalita_lng) ? `${clean(payload.lokalita_lat)}, ${clean(payload.lokalita_lng)}` : "-"}`,
    "",
    "Parametre:",
